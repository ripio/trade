...
public class UpdateOrder extends WebSocketClient {
    
    private static String apiKey;
    private static String secretKey;
    
    public UpdateOrder(URI serverUri) {
        super(serverUri);
    }
    ...
    public void onOpen(ServerHandshake handshakedata) {
        ...
        bodyParams.addProperty("price", "350000");
        bodyParams.addProperty("amount", "0.02");
        
        String signature = generateSignature(timestamp, bodyParams);
        
        JsonObject request = new JsonObject();
        request.addProperty("id", "req-update-001");
        ...
        System.out.println("\nSending update order request: " + new Gson().toJson(request));
    ...
    private static String generateSignature(long timestamp, JsonObject bodyParams) {
        try {
            ...
            SecretKeySpec secretKey = new SecretKeySpec(UpdateOrder.secretKey.getBytes(StandardCharsets.UTF_8), "HmacSHA256");
    ...
    public static void main(String[] args) {
        ...
        try {
            UpdateOrder client = new UpdateOrder(new URI("wss://ws-api.ripio.com"));
            client.connect();
        } catch (Exception e) {
            e.printStackTrace();
        }
